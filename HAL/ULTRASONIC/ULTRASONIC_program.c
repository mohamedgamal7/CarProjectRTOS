...
/* Static function to initialize Ultrasonic GPIO pins and configurations */
static void Ultrasonic_GPIO_Init(void)
{
    GPIO_PORTC_AFSEL_R |= (1 << 4);   // Enable alternate function for PC4
    GPIO_PORTC_PCTL_R |= (0x7 << 4 * 4); // Configure PC4 for timer input
}

/* Static function to initialize the Ultrasonic timer for measurements */
...
/* Static function to trigger a measurement */
static void trigger_measurement(void)
{
    DIO_Write(PORTB, P0, HIGH); // Set PB0 to high for triggering
    delay_us(15);               // Delay for 15 microseconds
    DIO_Write(PORTB, P0, LOW);  // Set PB0 back to low
}
...
float Measure_Distance(void)
{
    ...
    trigger_measurement(); // Trigger the measurement

    // Wait for the rising edge and handle timeout
    ...
    // Wait for the falling edge and handle timeout
    while (!TimerIntStatus(WTIMER_0, CAEIM) && --timeOut);
    if (timeOut <= 0)
        return -1; // Timeout
    ...
    Period = (Rise - Fall) / 16000000.0f; // Calculate time period in seconds
    Distance = (Period * 34300U) / 2U; // Calculate distance based on speed of sound
